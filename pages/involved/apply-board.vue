    ...
    <v-row class="mt-4">
      <v-col cols="12" md="7">
        ...
        >
          ...
          <ul>
            <li
              v-for="(responsibility, resindex) in position.responsibilities"
              :key="resindex"
              class="text-body-1"
            >
              {{ responsibility }}
          ...
          <ul>
            <li
              v-for="(requirement, reqindex) in position.requirements"
              :key="reqindex"
              class="text-body-1"
            >
              {{ requirement }}
      ...
      <v-col cols="12" md="5">
        <v-card class="pa-1" flat>
          ...
          <v-card-text v-if="done" class="text-center">
            ...
            <div class="mt-2">
              Thank you for applying! You may also submit your resume to
              contact@lingox.org and include your full name and desired position
              in the subject line.
            </div>
            <v-btn
              class="mt-2"
...
const submit = async () => {
  ...
  try {
    await $axios.$post(
      "https://form-submission.harrychen.workers.dev/wYERlUcXJqvAYqXk",
      formData.value
    );
    done.value = true;
    form.value.reset();
